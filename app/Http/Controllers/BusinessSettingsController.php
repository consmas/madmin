...
class BusinessSettingsController extends Controller
{
    ...
    public function payment_index()
    {
        ...
        $data_values = Setting::whereIn('settings_type', ['payment_config'])->whereIn('key_name', ['ssl_commerz', 'paypal', 'stripe', 'razor_pay', 'senang_pay', 'paytabs', 'paystack', 'paymob_accept', 'paytm', 'flutterwave', 'liqpay', 'bkash', 'mercadopago', 'roho_credit'])->get();
    ...
    public function payment_config_update(Request $request)
    {
        ...
        $validation = [
            'gateway' => 'required|in:ssl_commerz,paypal,stripe,razor_pay,senang_pay,paytabs,paystack,paymob_accept,paytm,flutterwave,liqpay,bkash,mercadopago,roho_credit',
        ...
        } elseif ($request['gateway'] == 'roho_credit') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'api_key' => 'required_if:status,1',
                'webhook_signing_secret' => 'nullable',
        ...
        if ($request->type == 'user_app') {
            ...
            if ($activationMode && $activationMode->value == 1 && $driveMondBaseUrl && $driveMondBaseUrl->value != null) {
                ...
                $response = Http::post($driveMondBaseUrl->value . '/api/store-configurations', [
                    'mart_business_name' => $name->value ?? 'ConsMas',
